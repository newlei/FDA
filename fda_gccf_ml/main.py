...
task_optimizer = torch.optim.Adam(list(model.embed_user.parameters()) + \
                            list(model.embed_item.parameters()) ,lr=0.002)
...
for epoch in range(87):
    model.train()  
    start_time = time.time()
    print('negative samping, about 10 minute')
    ...
    loss_str='loss' 
    loss_str+=' task:'+str(train_loss_task)#+'\t noise:'+str(train_loss_noise)+'\t regu'+str(train_loss_sample)
